...
load('E:\hww\ESVRGC\simulation_difflen\10.6SMk3d0.03N1000_4.mat'); 
y = XX';
...
sequence_length = 1;
num_shift = 2;
x2 = zeros(size(MinMaxData,1)/2,sequence_length,size(y,2));
y2 = zeros(size(MinMaxData,1)/2,size(y,2));
% x2 = zeros(floor((size(y,1)-sequence_length)/num_shift),sequence_length,size(y,2));
% y2 = zeros(floor((size(y,1)-sequence_length)/num_shift),size(y,2));
% for p =1:num_shift:size(MinMaxData,1)
%     
%     if (p+ sequence_length + num_shift >size(MinMaxData,1))
%         break;
%     end
%     for i = p:1:p+num_shift - 1
%         
%         x2(p,i,:) = [MinMaxData(i:i + sequence_length-1,:)];
%     
%         y2(p+1,i) = [MinMaxData(i + sequence_length,:)];
%     end
...
for p =1:num_shift:size(MinMaxData,1)
    ...
    x2((p+1)/2,:,:) = [MinMaxData(p,:)];
    y2((p+1)/2,:) = [MinMaxData(p+1,:)];
%     if(num_shift>1)
%         for i = p+1:1:p+sequence_length-1
%             x2(i,:,:) = [MinMaxData(i:i + sequence_length-1,:)];
%             y2(i,:) = [MinMaxData(i + sequence_length,:)];
%         end
%     end
...
%     C=10000;
%     e=1;
%     ker = struct('type','gauss','width',0.1);
%     svm=svmTrain('svr_epsilon',x_111',tmp_y',ker,C,e);
%     yd=svmSim(svm,x_111');
%     var_denominator(1,k) = var(yd'-tmp_y);
%     mdl = fitrsvm(x_111,tmp_y,'KernelFunction','gaussian','KernelScale',1);



%     mdl = fitrsvm(x_111,tmp_y,'KernelFunction','gaussian');
%     
%     
%     
% %     mdl = fitrsvm(x_111,tmp_y);
%     yd = resubPredict(mdl);
%     var_denominator(1,k) = var(yd-tmp_y);
...
%         ker = struct('type','gauss','width',0.1);
%         svm=svmTrain('svr_epsilon',x_111',tmp_y',ker,C,e);
%         yd1=svmSim(svm,x_111');
%         granger_matrix(j,k)  = var(yd1'-tmp_y);
%         
...
% %         mdl = fitrsvm(x_111,tmp_y);
%         yd = resubPredict(mdl);
%         granger_matrix(j,k)  = var(yd-tmp_y);
...
% y_label = A(:);
% y_new = granger_matrix(:)./max(max(granger_matrix));
% [X,Y] = perfcurve(y_label,y_new);
% plot(X,Y);
...
y_label = reshape(A,1,size(A,1)*size(A,1));
y_new = reshape(granger_matrix',1,size(A,1)*size(A,1))./max(max(granger_matrix'));
plotroc(y_label,y_new);
% [X,Y] = plotroc(y_label,y_new);
% plot(X,Y);
% xlabel('False positive rate') ;
% ylabel('True positive rate');
% title('ROC');